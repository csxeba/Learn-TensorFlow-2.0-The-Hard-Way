...
class History:
    ...
    def incorporate(self, other: "History", do_reduction: bool):
        if other.keys != self.keys:
            raise ValueError("Cannot incorporate other history object with a different header!")
        for key in self.keys:
            item = other._logs[key]
            if not item:
                continue
            if do_reduction:
                self._logs[key].append(np.mean(item))
            else:
                self._logs[key].extend(item)
